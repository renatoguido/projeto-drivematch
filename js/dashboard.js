...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!panel) {
        console.warn('❌ Painel de acessibilidade não encontrado!');
        return;
    }

    console.log('✅ Botão e painel encontrados!');
    ...
    toggleBtn.addEventListener('click', function (e) {
        ...
        console.log('📂 Painel:', isHidden ? 'fechado' : 'aberto');
    });
    ...
    if (closeBtn) {
        closeBtn.addEventListener('click', function (e) {
            ...
            console.log('❌ Painel fechado pelo X');
        });
    }

    // Fechar clicando fora
    document.addEventListener('click', function (e) {
        if (panel.classList.contains('acc-hidden')) return;

        const clicouNoPainel = panel.contains(e.target);
        const clicouNoBotao = toggleBtn.contains(e.target);

        if (!clicouNoPainel && !clicouNoBotao) {
            panel.classList.add('acc-hidden');
            panel.setAttribute('aria-hidden', 'true');
            console.log('❌ Painel fechado por clique fora');
        }
    });
});
...
function resetAccessibility() {
    ...
    updateUI();
    showToast('♿ Configurações de acessibilidade redefinidas');
}

// Exportar funções para uso global
window.changeFontSize = changeFontSize;
window.toggleWideSpacing = toggleWideSpacing;
window.toggleLineHeight = toggleLineHeight;
window.toggleHighContrast = toggleHighContrast;
window.toggleMonochrome = toggleMonochrome;
window.toggleHideImages = toggleHideImages;
